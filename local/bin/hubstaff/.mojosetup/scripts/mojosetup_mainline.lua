...
local function install_manifests(desc, key)
    ...
    local package =
    {
        ...
        autostartitems = MojoSetup.install.autostartitems,
...
local function freedesktop_menuitem_filename(pkg, idx, item)  -- only for Unix.
    if (item.filename ~= nil) then
        return MojoSetup.metadatadir .. "/" .. item.filename .. ".desktop"
    end
    ...
    if pkg.desktopmenuitems ~= nil then
        for i,v in ipairs(pkg.desktopmenuitems) do
            ...
            elseif MojoSetup.info.platform == "beos" then
                ...
                local fname = freedesktop_menuitem_filename(pkg, i, v)
                if not MojoSetup.platform.uninstalldesktopmenuitem(fname) then
                    MojoSetup.fatal(_("Failed to uninstall desktop menu item"))
                else
                    MojoSetup.platform.runscript( "meta/xdg-update-desktop-database", false )
                end
            end
        end
    end
...
--        cmdlinearg = '"' .. string.gsub(string.gsub(cmdlinearg, "\"","\\\""), "%%", "%%%%") .. '"'
    ...
    if item.no_display ~= nil then
        addpair("NoDisplay", item.no_display and "true" or "false")
    end
    ...
    local fname = freedesktop_menuitem_filename(pkg, idx, item)
    local perms = "0644"  -- !!! FIXME
...
-- Autostart item
local function freedesktop_autostartitem_filename(pkg, idx, item)  -- only for Unix.
    if (item.filename ~= nil) then
        return MojoSetup.metadatadir .. "/autostart/" .. item.filename .. ".desktop"
    end

    local vendor = string.gsub(pkg.vendor, "[. ,]", "_")
    local fname = vendor .. "-" .. pkg.id .. "_" .. idx .. ".desktop"
    return MojoSetup.metadatadir .. "/autostart/" .. fname
end


local function uninstall_autostart_items(pkg)
    -- !!! FIXME: GUI progress?
    if pkg.autostartitems ~= nil then
        for i,v in ipairs(pkg.autostartitems) do
            if MojoSetup.info.platform == "windows" then
                MojoSetup.fatal(_("Unimplemented"))  -- !!! FIXME: write me.
            elseif MojoSetup.info.platform == "macosx" then
                MojoSetup.fatal(_("Unimplemented"))  -- !!! FIXME: write me.
            elseif MojoSetup.info.platform == "beos" then
                ...
                local fname = freedesktop_autostartitem_filename(pkg, i, v)
                if not MojoSetup.platform.runscript( "meta/xdg-autostart-item", false, "uninstall", fname ) then
                    MojoSetup.fatal(_("Failed to uninstall autostart item"))
                end
            end
        end
    end
...
local function install_freedesktop_autostarttem(pkg, idx, item)  -- only for Unix.
    local dest = MojoSetup.destination
    ...
    local fname = freedesktop_autostartitem_filename(pkg, idx, item)
    local perms = "0644"  -- !!! FIXME
    local key = MojoSetup.metadatakey
    local desc = MojoSetup.metadatadesc

    --MojoSetup.logdebug("Install FreeDesktop autostart file")
    --MojoSetup.logdebug(fname)
    --MojoSetup.logdebug(str)
    install_parent_dirs(fname, key)
    install_file_from_stringtable(fname, t, perms, desc, key)
    if not MojoSetup.platform.runscript( "meta/xdg-autostart-item", false, "install", fname ) then
        MojoSetup.fatal(_("Failed to install autostart item"))
    end
end


local function install_autostart_items(pkg)
    -- !!! FIXME: GUI progress?
    if pkg.autostartitems ~= nil then
        for i,item in ipairs(pkg.autostartitems) do
            if not item.disabled then
                if MojoSetup.info.platform == "windows" then
                    MojoSetup.fatal(_("Unimplemented"))  -- !!! FIXME: write me.
                elseif MojoSetup.info.platform == "macosx" then
                ...
                elseif MojoSetup.info.platform == "beos" then
                    ...
                    install_freedesktop_autostarttem(pkg, i, item)
                end
            end
        end
    end
...
local function is_valid_destination( dest )
    -- if its a writable directory, its usable
    if MojoSetup.platform.isdir(dest) then
        return MojoSetup.platform.writable(dest)
    -- if its a file, its not
    elseif MojoSetup.platform.isfile(dest) then
        return false
    -- it might be a new dir inside an existing dir
    else
        local path = string.gsub(dest,"/+[^/]*$","")
        if path == "" or path == dest then
            return false
        else
            return is_valid_destination(path)
        end
    end
end
...
local function do_install(install)
    ...
    MojoSetup.installed_autostart_items = false
    ...
    -- Autostart items equire uninstall so we don't clutter
    --  the system with no option for reversal later.
    -- !!! FIXME: will miss menu items that are Setup.Option children...
    if (install.autostartitems ~= nil) and (not install.support_uninstall) then
        MojoSetup.fatal(_("BUG: Setup.AutostartItem requires support_uninstall"))
    end
    ...
    elseif destcmdline ~= nil then
        ...
        if recommended_cfg ~= nil then
            ...
            for i,v in ipairs(recommended_cfg) do
                if is_valid_destination(v) then
        ...
        -- silent install, we cannot prompt the user, so use the defalt location (if present) otherwise panic
        if( silent ) then
           if( recommend ~= nil ) then
                set_destination(recommend[1])
           else
                panic("No default destination specified for silent install")
           end
        else
    ...
    stages[#stages+1] = function(thisstage, maxstage)
        ...
        local function process_option(option)
            ...
            if option.autostartitems ~= nil then
                for i,item in ipairs(option.autostartitems) do
                    if install.autostartitems == nil then
                        install.autostartitems = {}
                    end
                    install.autostartitems[#install.autostartitems+1] = item
                end
            end
        end
    ...
    stages[#stages+1] = function(thisstage, maxstage)
        ...
        if install.autostartitems ~= nil then
            install_autostart_items(install)
            MojoSetup.installed_autostart_items = true
        end
    ...
    MojoSetup.installed_autostart_items = nil
...
local function real_revertinstall()
    ...
    if MojoSetup.installed_autostart_items then
        uninstall_autostart_items(MojoSetup.install)
    end
...
local function uninstaller()
    ...
    if uninstall_permitted then
        ...
        uninstall_autostart_items(package)
